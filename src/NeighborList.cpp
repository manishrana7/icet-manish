...
void NeighborList::build(const Structure &conf)
{
    ...
    std::vector<size_t> unitCellExpanse(3);
    ...
    for (size_t n1 = 0; n1 < unitCellExpanse[0] + 1; n1++)
    {
        for (size_t n2 = -unitCellExpanse[1]; n2 < unitCellExpanse[1] + 1; n2++)
        {
            for (size_t n3 = -unitCellExpanse[2]; n3 < unitCellExpanse[2] + 1; n3++)
            {
                for (size_t m = -1; m < 2; m += 2)
                {
