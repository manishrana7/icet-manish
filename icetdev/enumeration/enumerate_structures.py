...
def get_reduced_hnf_matrices(N, A, rotations_inv):
    '''
...
def get_snfs_and_dangerous_rotations(hnfs, A, Ainv, rotations_inv):
    ...
    hnfs : list of ndarrays
        HNF matrices, typically calculated with get_reduced_hnf_matrices()
    A : ndarray
        Parent lattice (basis vectors listed column wise).
    Ainv : ndarray
...
def get_enumerated_structures(atoms, size, subelements):
    ...
    for N in sizes:
        hnfs = get_reduced_hnf_matrices(N, A, rotations_inv)
