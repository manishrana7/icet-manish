...
def test_mi_int_list_and_dict(atoms, subelements, cutoffs, allowed_sites):
    ...
    assert (np.array(cv_int) == np.array(
        cv_list)).all, "Error: clustervectors were not equal for an equivalent Mi with: type(Mi) == int and type(Mi)==list"
    assert (np.array(cv_int) == np.array(
        cv_dict)).all, "Error: clustervectors were not equal for an equivalent Mi with: type(Mi) == int and type(Mi)==dict"
