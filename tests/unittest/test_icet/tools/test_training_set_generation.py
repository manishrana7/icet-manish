...
class TestStructurePoolGenerationWithAnnealing(unittest.TestCase):
    ...
    def test_structure_selection_annealing_correct_steps_and_number_of_structures(self):
        """
        Test that the number of structure is correct for a few cases
        """
        (inds, traj) = structure_selection_annealing(self.cs,
                                                     self.structure_pool,
                                                     10,
                                                     int(1e1))
        assert len(inds) == 10
    ...
    def test_structure_annealing_with_initial_indices(self):
        ...
        """
        initial_indices = [f for f in range(10)]
        (inds, _) = structure_selection_annealing(self.cs,
                                                  self.structure_pool,
                                                  10,
                                                  0,
                                                  initial_indices=initial_indices)
    ...
    def test_structure_selection_annealing_minimization_condition_number(self):
        """
        Test that the condition number is minimized
        """
        (_, traj) = structure_selection_annealing(self.cs,
                                                  self.structure_pool,
                                                  10,
                                                  int(1e3))
        assert traj[-1] < traj[0]

    def test_structure_selection_annealing_gives_correct_last_indices(self):
        """
        Test that we return the correct structure indices
        """
        initial_indices = [f for f in range(10)]
        (end_inds, traj) = structure_selection_annealing(self.cs,
                                                         self.structure_pool,
                                                         10,
                                                         int(1e3),
                                                         initial_indices=initial_indices)
    ...
    def test_structure_selection_annealing_with_base_input(self):
        """
        Test that we return the correct structure indices with base input
        """
