...
The first function, :func:`generateRandomStructure`, takes an :class:`ASE
...
The second function, :func:`generateCVSet`, generates cluster vectors for ``n``
...
<icetdev.cluster_space.ClusterSpace>` object ``clusterspace``. Specifically,
this is achieved by first calling the :func:`generateRandomStructure` function,
...
The third function, :func:`getColumnCorrelation`, calculates the correlation
...
The fourth function, :func:`assertNoCorrelation`, checks that the correlations
...
columns with help of the :func:`getColumnCorrelation`, which was defined
...
``clusterspace``, is initiated using the :class:`ASE Atoms` object
...
``atoms_row``, ``subelements`` and ``clusterspace`` as additional input
arguments. The :class:`list` of cluster vectors thus obtained is then fed into
the :func:`assertNoCorrelation` function, that was descibed earlier, to check
